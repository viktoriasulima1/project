...
export default async function WorkOrdersPage({ searchParams }: { searchParams: Promise<{ field?: string; plan?: string; view?: string }> }) {
  ...
  return <><Topbar title={t('title')} subtitle={t('subtitle')} farmName={farm.name}/><main className={styles.page}>
    <div className={styles.actions}><a className={styles.button} href="/work-orders?view=today">{t('tabs.today')}</a><a className={styles.button} href="/work-orders">{t('tabs.allWork')}</a><a className={styles.button} href="/planning">{t('tabs.seasonPlan')}</a></div>
