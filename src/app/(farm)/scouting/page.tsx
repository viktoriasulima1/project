...
export default async function ScoutingPage({ searchParams }: { searchParams: Promise<{ field?: string }> }) {
  ...
  return <><Topbar title={t('page.title')} subtitle={t('page.subtitle')} farmName={farm.name}/><main className={styles.page}>
    ...
    <section className={styles.card}><h2>{t('page.recentVisits')}</h2>{recent.length ? <ul className={styles.visits}>{recent.map((v)=><li className={styles.visit} key={v.id}><strong>{v.fieldName}</strong> · {v.visitDate.toLocaleString(locale)} · {v.observations.length} observation(s)<br/><span className={styles.muted}>{v.observations.map((o)=>`${o.issueType} (${o.status}, ${o.severity})`).join(' · ')}</span></li>)}</ul> : <p className={styles.muted}>{t('page.noVisits')}</p>}</section>
  </main></>;
}
